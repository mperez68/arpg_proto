...
class GameEngine {
	...
	drawIterator(items) {
        for (var i = 0; i < items.length; i++) {
			...
			let drawFlag = true;
			if (Math.abs(this.camera.x + (PARAMS.PAGE_WIDTH / 2) - items[i].x) > PARAMS.PAGE_WIDTH * 1.1) drawFlag = false;
			if (Math.abs(this.camera.y + (PARAMS.PAGE_WIDTH / 2) - items[i].y) > PARAMS.PAGE_HEIGHT * 1.1) drawFlag = false;
